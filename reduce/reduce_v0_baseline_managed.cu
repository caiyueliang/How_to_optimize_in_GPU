...
int main(int argc, char **argv) {
    ...
    // 获取并打印传入的参数
    const int block_num = std::atoi(argv[1]);
    ...
    if (version == 0) {   
        ...
        reduce3<<<Grid, Block, block_size*sizeof(float)>>>(a, out);
    } else if (version == 6) {   
        reduce6<<<Grid, Block, block_size*sizeof(float)>>>(a, out);
    } else if (version == 7) {   
        // reduce7<><<<Grid, Block, block_size*sizeof(float)>>>(a, out);
        // const int NUM_PER_BLOCK = N / block_num;
        // const int NUM_PER_THREAD = N / block_num / block_size;
        reduce7<THREAD_PER_BLOCK, 1><<<Grid, Block>>>(a, out, N);
    } else {
        reduce0<<<Grid, Block, block_size*sizeof(float)>>>(a, out);
        reduce1<<<Grid, Block, block_size*sizeof(float)>>>(a, out);
        reduce2<<<Grid, Block, block_size*sizeof(float)>>>(a, out);
